class UsersController < ApplicationController
  ...
  include Lister
	SearchPairs = [['a','user_name'],['m',SQL_FULL_NAME],
	               ['f','first_name'], ['l','last_name'], ['e','email'],['s','state'],
	               ['n','notes']]
  ...
  def index
    ...
    def_search_rule 's_is_sql' do |search_for|
      condition_string = %Q| state = "#{search_for}" |
      condition_string << %Q| or state = "#{StateNameHash[search_for]}" | if StateNameHash[search_for]
      ["#{condition_string}"]
    end
